...
class installCore extends kernel
{
    ...
    {
        define( "CORE_INCLUDE_DIR", CORE_DIR.( file_exists( CORE_DIR."/include_v5" ) && version_compare( PHP_VERSION, "5.0", ">=" ) ? "/include_v5" : "/include" ) );
        if ( isset( $_POST['dbname'], $_POST['dbuser'] ) && isset( $_POST['dbhost'] ) && isset( $_POST['data'] ) )
        {
            ...
            $GLOBALS['_POST']['uname'] = $_POST['dbuser'];
            $GLOBALS['_POST']['pwd'] = $_POST['dbpass'];
            $GLOBALS['_POST']['prefix'] = $_POST['prefix'] ? $_POST['prefix'] : "sdb_";
            $GLOBALS['_POST']['use_demo'] = $_POST['data'] == "true" ? TRUE : FALSE;
            $GLOBALS['_POST']['stimezone'] = $_POST['stimezone'] ? $_POST['stimezone'] : 8;
            parent::kernel( );
    ...
    public function service_install_res( $res = "false", $rsp = NULL )
    {
    ...
    public function service_install_mode( )
    {
    ...
    public function run( )
    {
        $GLOBALS['_GET']['step'] = isset( $_GET['step'] ) ? $_GET['step'] : 1;
    ...
    public function step_checkdb( )
    {
        ...
        $database = array( );
        while ( ( $result = mysql_query( "show databases" ) ) && ( $row = mysql_fetch_array( $result, MYSQL_ASSOC ) ) )
        {
    ...
    public function step_setdb( )
    {
        ...
        else
        {
            ...
            if ( RANDOM_HOME )
            {
                $arr['#(define\\(\\s*['\"]HOME_DIR['\"]\\s*,\\s*)[^;]+;#i'] = "\\1dirname(__FILE__).'/home_".substr( md5( microtime( ) ), 3, 6 )."');";
            }
    ...
    public function do_install( $full_install = TRUE )
    {
        if ( !$full_install )
        {
        ...
        if ( RANDOM_HOME )
        {
            if ( !is_dir( HOME_DIR ) && !mkdir( HOME_DIR, 511 ) )
            {
    ...
    public function step_full_install( )
    {
        set_time_limit( 0 );
        if ( $this->loadConfig( ) )
        {
    ...
    public function step_install_mysql_db( )
    {
        $this->loadConfig( );
        if ( constant( "DB_HOST" ) )
        {
            ...
            else if ( preg_match( "/[0-9\\.]+/is", mysql_get_server_info( $lnk ), $match ) )
            {
                $dbver = $match[0];
            }
        }
    ...
    public function step_install_init_db( )
    {
        $this->loadConfig( );
    ...
    public function get_exec_sql( $file, $errormsg = "安装出错" )
    {
        if ( $this->db_connect( ) )
        {
    ...
    public function step_install_demo_db( )
    {
        $this->loadConfig( );
    ...
    public function step_install_plugins( )
    {
        $this->loadConfig( );
        ...
        foreach ( $apps as $k => $v )
        {
            if ( $v['plugin_ident'] )
            {
                ...
                else if ( $_POST['install_stat'] && $appmgr->load( "shopex_stat" ) )
                {
                    $appmgr->install( $v['plugin_ident'] );
    ...
    public function step_install_success( )
    {
        if ( !$this->db_connect( ) )
        {
            echo "安装初始化数据失败!|fail";
    ...
    public function step_install_fake_html( )
    {
        if ( !$this->db_connect( ) )
        {
            echo "安装初始化数据失败!|fail";
    ...
    public function step_install_done( )
    {
        file_put_contents( BASE_DIR."config/install.lock", "If you want to reinstall system, delete this file!" );
        echo "success";
    }
    ...
    public function install_basic_db( )
    {
        $o =& $this->loadModel( "utility/schemas" );
        $aDb = $o->get_system_schemas( );
        $aDb = array_merge( array(
            "settings" => $aDb['settings'],
            "plugins" => $aDb['plugins']
        ), $aDb );
    ...
    public function step_reinstall( )
    {
        $this->step_install( );
    }
    ...
    public function _checkDirPerm( $dir, &$arr, $r = FALSE )
    {
        $arr[] = array(
            "path" => $dir,
            "is_writable" => is_writable( $dir )
        );
        if ( $r && ( $handle = opendir( $dir ) ) )
        {
