...
class mdl_serverinfo
{
    ...
    public function test_server_req( &$score )
    {
        ...
        else
        {
            ...
            else if ( $db_ver = mysql_get_server_info( ) )
            {
                define( "DB_HOST", "" );
            }
            else
            ...
            $items['ZEND Optimizer2.5.7以上'] = array(
                "value" => $value,
                "result" => $rst
            );
            if ( !$rst )
...
if ( !defined( "IN_INSTALLER" ) )
{
    ...
    else if ( isset( $_GET['phpinfo'] ) && !file_exists( dirname( __FILE__ )."/../config/config.php" ) )
    {
        phpinfo( );
    }
