...
function widget_goods( &$setting, &$system )
{
    ...
    $limit = 0 < intval( $setting['limit'] ) ? intval( $setting['limit'] ) : 6;
    ...
    if ( 1 < $setting['columNum'] )
    {
        ...
        for ( ; $i <= $setting['columNum']; ++$i )
        {
            ...
            $result['link'][$i - 1] = $system->mkUrl( "gallery", $setting['view'], array(
                implode( ",", $filter[$i]['cat_id'] ),
                $oSearch->encode( $filter[$i] ),
                $setting['goods_orderby'] ? $setting['goods_orderby'] : 0
            ) );
    ...
    else
    {
        ...
        $oSearch = $system->loadModel( "goods/search" );
        $result['link'] = $system->mkUrl( "gallery", $setting['view'], array(
