...
class passport_phpwind7 extends modelFactory
{

    public $passport_name = "PhpWind论坛V7.0";
    public $passport_memo = "此功能可进行第三方系统（譬如论坛系统）用户同步与登录同步设置（Phpwind论坛最新版本已内置本系统接口，其它系统请根据文档自行编写）；";
    public $_config = NULL;
    public $forward = 0;
    public $name = "phpwind7";
    ...
    public function getoptions( )
    {
        return array(
            ...
            "conntype" => array(
                "label" => "本网站作为：",
                "type" => "radio",
                "options" => array( "service" => "服务器端", "client" => "客户端" )
            )
        );
    }
    ...
    public function login( $userId, $rurl )
    {
        ...
        $Eforward = str_replace( array( "\x00", "%00", "\r" ), "", $forward );
        $Eforward = preg_replace( array( "/[\\x00-\\x08\\x0B\\x0C\\x0E-\\x1F]/", "/&(?!(#[0-9]+|[a-z]+);)/is" ), array( "", "&amp;" ), $Eforward );
        $verify = md5( "login{$userdb_encode}{$Eforward}{$key}" );
    ...
    public function _StrCode( $string, $action = "ENCODE" )
    {
    ...
    public function ClientUserAction( $action, $userdb, $forward = "" )
    {
    ...
    public function ServerClient( $action )
    {
        ...
        if ( $this->_config['conntype'] == "client" )
        {
            ...
            $url = substr( $this->_config['URL'], -1 ) == "/" ? $this->_config['URL'] : $this->_config['URL']."/";
            $url .= $toUrl;
            header( "Location:".$url );
            exit( );
        }
