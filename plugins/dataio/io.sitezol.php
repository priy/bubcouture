...
class io_sitezol
{
    ...
    public $name = "ZOL数据导入(输入产品参数页链接)";
    public $importforObjects = "goods";
    public $developing = TRUE;
    ...
    public function import_row( &$handle )
    {
        ...
        if ( $handle['count'] == 0 )
        {
            ...
            $net =& $system->loadModel( "utility/http_client" );
            if ( $result = $net->get( $handle['url'] ) )
            {
                $p = $this->charset->local2utf( $results, "zh" );
