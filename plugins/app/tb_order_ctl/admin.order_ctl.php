...
class admin_order_ctl extends ctl_order
{
    ...
    public function _detail( )
    {
        return array(
            "detail_info" => array(
                "label" => __( "基本信息" ),
                "tpl" => dirname( __FILE__ )."/view/order/order_detail.html"
            )
        );
    }

    public function detail( $object_id, $func = NULL )
    {
        $this->template_dir = CORE_DIR."/admin/view/";
    ...
    public function toPrint( )
    {
        ...
        foreach ( $aInput as $orderid )
        {
            ...
            $this->pagedata['pages'][] = $this->_fetch_compile_include( dirname( __FILE__ )."/view/order/orderprint.html", array(
                "order" => $aData
            ) );
        }
    ...
    public function detail_info( $order_id )
    {
        ...
        $tb_pay_time = 604800 + $aOrder['createtime'] - time( );
        $this->pagedata['order_status'] = $order_status;
        $this->pagedata['paytb_tiem'] = $this->get_m_d_y( $tb_pay_time );
        $this->pagedata['confirm_time'] = $this->get_m_d_y( 864000 + $aOrder['delivery_time'] - time( ) );
    ...
    public function get_m_d_y( $tb_pay_time )
    {
        $day = floor( $tb_pay_time / 86400 );
        $tmp_hour = $tb_pay_time % 86400;
    ...
    public function set_order_price( $order_id )
    {
    ...
    public function delay_delivery_time( $order_id )
    {
    ...
    public function showConsignFlow( $order_id )
    {
    ...
    public function toDelivery( $order_id )
    {
        include_once( "delivercorp.php" );
        ...
        $GLOBALS['_POST']['logi_name'] = $aCorp['name'];
    ...
    public function addtradenote( $order_id )
    {
        require_once( "mdl.tborder.php" );
        ( );
    ...
    public function detail_tbdelivery( $order_id )
    {
        $db = $this->system->database( );
    ...
    public function toaddtradenote( )
    {
        $this->template_dir = CORE_DIR."/admin/view/";
        require_once( "mdl.tborder.php" );
        ( );
        ...
        foreach ( $_POST['order_id'] as $key => $value )
        {
            if ( isset( $_POST['result'][$key] ) )
            {
                ...
                if ( isset( $returnmsg['error_response']['sub_msg'] ) )
                {
                    $this->splash( "failed", "index.php?ctl=plugins/order_ctl&act=addtradenote&p[0]=".$_POST['tid'], $returnmsg['error_response']['sub_msg'] );
                    exit( );
                }
                if ( $value )
                {
    ...
    public function get_order_refund_status( $refund_id )
    {
    ...
    public function getBytbship( $type )
    {
