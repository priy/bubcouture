...
class taobao_action extends kernel
{
    ...
    public function set_order_price( )
    {
    ...
    public function delay_delivery_time( )
    {
    ...
    public function taobao_error_list( $code )
    {
        $error_code = array( "1" => "系统错误", "14002" => "查询退款信息失败", "14003" => "查询物流信息失败", "14004" => "卖家同意退款协议失败", "14005" => "卖家拒绝退款协议失败", "14006" => "卖家确认退款失败", "14007" => "卖家拒绝退款失败", "14008" => "卖家必须上传发货凭证", "14009" => "买家又修改了退款协议", "14010" => "退款已经关闭", "14011" => "创建留言失败", "14014" => "零元退款金额校验失败", "14015" => "对不起，您已经发起了退款", "14016" => "收货地址不能为空", "14017" => "由于红包金额不支持部分退款，您不能进行本次操作", "14018" => "时间戳丢失", "14019" => "对不起，操作失败，本笔退款会导致红包拆分", "14020" => "参数错误", "14021" => "无法要求小二介入", "14101" => "您不是交易买家", "14102" => "您不是交易卖家", "14103" => "目前您不能退款发货", "14104" => "目前您不能发起退款", "14105" => "目前您不能修改退款协议", "14106" => "你不是交易双方", "14108" => "客服初审完成，不能修改退款协议", "14109" => "目前您不能同意或拒绝退款协议", "14110" => "目前您不能同意或拒绝打款给买家", "14111" => "退款已经结束，您不能再提交留言凭证", "14112" => "你要求的退款金额不合法", "14115" => "物流公司为空", "14116" => "非法物流方式", "14117" => "没有退款记录", "16001" => "获取订单失败", "16002" => "只有商品的买家才能确认收货", "16003" => "已经确认收货，无须再次确认", "16004" => "订单存在退款(主订单退款或一部分子订单有退款)", "16005" => "未退款的子订单可以确认收货，前提是其父订单必须为正在退款的状态", "12001" => "系统错误", "12003" => "不能关闭交易", "12004" => "没有权限", "12005" => "用户没有选择关闭交易理由", "13001" => "系统错误", "11001" => "系统错误", "11003" => "没有权限", "11004" => "用户没有选择关闭交易理由", "11005" => "关闭所有交易失败", "11008" => "总价不含邮费需要大于0" );
        if ( $error_code[$code] )
        {
    ...
    public function set_pay_status( )
    {
        ...
        {
            $GLOBALS['_GET']['pay_time'] = date( "Y-m-d H:i:s" );
        }
    ...
    public function goods_confirm( )
    {
        ...
        $tmpdata = $this->db->selectrow( "SELECT order_id FROM sdb_order_items WHERE taobao_order=".$_GET['biz_order_id'] );
        if ( $tmpdata && !$this->db->select( "SELECT order_id FROM sdb_order_items WHERE (status='wait' or status ='refund') AND order_id=".$tmpdata['order_id'] ) )
        {
    ...
    public function close_dialog( $op = "admin" )
    {
        if ( $op == "admin" )
        {
    ...
    {
        $list = array( "goods_confirm" => "shop", "close_order" => "admin", "set_order_price" => "admin", "delay_delivery_time" => "admin", "refund_create" => "shop" );
        return $list[$key];
    }

    public function redirect_tb( $redirect )
    {
        if ( $redirect == "admin" )
        {
    ...
    public function get_refund_status( $num )
    {
        $list = array( "10" => 2, "20" => 6, "21" => 1, 6 => 3, 2 => 8, 5 => 7, 3 => 5, 4 => 0 );
        if ( $list[$num] )
        {
            return $list[$num];
        }
