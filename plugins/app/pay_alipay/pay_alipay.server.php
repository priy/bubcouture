...
class pay_alipay extends paymentPlugin
{
    ...
    public function pay_alipay_callback( $in, &$paymentId, &$money, &$message, &$tradeno )
    {
        ksort( $in );
        foreach ( $in as $k => $v )
        {
        ...
        if ( trim( $ikey ) == "" )
        {
            $ikey = "afsvq2mqwc7j0i69uzvukqexrzd0jq6h";
        }
        if ( md5( $mac.$ikey ) == $in['sign'] )
        {
            ...
            else if ( $in['trade_status'] == "TRADE_FINISHED" )
            {
                echo "success";
                return PAY_SUCCESS;
            }
            else if ( $in['trade_status'] == "TRADE_SUCCESS" )
            {
                echo "success";
                return PAY_SUCCESS;
            }
            else if ( $in['trade_status'] == "WAIT_BUYER_PAY" )
            {
                echo "success";
                return PAY_IGNORE;
            }
