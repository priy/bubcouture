...
class api_1_0_goods extends shop_api_object
{
    ...
    public function set_goods_bn( $data )
    {
        safevar( $data );
    ...
    public function search_goods_detail( $data )
    {
        ...
        if ( $result['data_info']['pdt_desc'] )
        {
            ...
            unset( $this->data_info['pdt_desc'] );
            foreach ( $pdt_desc as $key => $value )
            {
                $result['data_info']['pdt_desc'][] = array(
                    "product_id" => $key ? $key : "",
                    "product_desc" => $value ? $value : ""
                );
            }
        }
        if ( $result['data_info']['spec_desc'] )
        {
            ...
            unset( $this->data_info['spec_desc'] );
        ...
        {
            $result['data_info']['goods_link'] = $this->system->realUrl( "product", "index", array(
                $data['goods_id']
        ...
        if ( $data['columns_join']['image'] )
        {
            ...
            {
                $result['data_info']['images'][] = array(
                    "image_id" => $value['gimage_id'],
                    "small" => $value['small'] ? $value['small'] : "",
                    "big" => $value['big'] ? $value['big'] : "",
                    "thumbnail" => $value['thumbnail'] ? $value['thumbnail'] : "",
                    "is_default" => $value['gimage_id'] == $image_default_id ? "true" : "false"
                );
            }
        }
        if ( $data['columns_join']['products_info'] && ( $products = $this->db->select( "select product_id,name,bn,price,store,marketable,last_modify from sdb_products where goods_id=".intval( $result['data_info']['goods_id'] ) ) ) )
        {
    ...
    public function get_goods_brand( )
    {
    ...
    public function add_goods_info( $data )
    {
        if ( !$data['type_id'] )
        {
        ...
        foreach ( json_decode( $data['products_info'], TRUE ) as $c_k => $c_v )
        {
            ...
            foreach ( $g_spec as $g_k => $g_v )
            {
                ...
                {
                    $t_spec_name_c[$spec_id][$ss] = array(
                        "spec_value" => $temp['spec_value'],
                        "spec_type" => $temp['spec_type'],
    ...
    public function goods_wltx_exp_list( $data )
    {
    ...
    public function search_deleted_goods_list( $data )
    {
    ...
    public function set_goods_marketable_succ( $data )
    {
        if ( !$data['goods_id'] )
        {
    ...
    public function search_goods_list_by_lastmodify( $data )
    {
        if ( $data['last_modify_st_time'] == "0" )
        {
    ...
    public function create_sell_log( $data )
    {
    ...
    public function update_goods_store( $data )
    {
    ...
    public function get_spec_list( $data )
    {
    ...
    public function update_goods_image( $data )
    {
    ...
    public function get_shop_goods_type( )
    {
    ...
    public function get_goods_props( $data )
    {
    ...
    public function delete_goods_images( $data )
    {
        if ( substr( $data['image_id'], -1 ) == "," )
        {
            $this->api_response( "fail", "image_id is error", $data );
        }
    ...
    public function delete_goods_image( $data )
    {
    ...
    public function update_goods_info( $data )
    {
        $aData = $data;
        $aData['uptime'] = time( );
        $aData['last_modify'] = time( );
    ...
    public function add_products_info( $c_v )
    {
        ...
        if ( substr( $c_v['spec_info'], -1 ) == ";" )
        {
            $this->api_response( "fail", "spec_info is error", $c_v );
        }
    ...
    public function convert_props( $data )
    {
        ...
        foreach ( $prd_data as $key => $value )
        {
            ...
            foreach ( $np['spec'] as $ke => $va )
            {
                ...
                {
                    $spec_desc[$ke][$np['spec_private_value_id'][$ke]] = array(
                        "spec_value" => $va,
                        "spec_type" => $tmp['spec_type'],
                        "spec_value_id" => $np['spec_value_id'][$ke],
                        "spec_image" => "",
                        "spec_goods_images" => ""
                    );
                }
    ...
    public function update_products_info( $data )
    {
        $this->tobedit( $data );
        $result['data_info'] = array(
            "products_id" => $data['products_id'],
    ...
    public function get_type_spec( $data )
    {
    ...
    public function del_images_byid( $data )
    {
