...
class api_products
{
    ...
    public function taobao_query_and_errorlog( $sql )
    {
        if ( !$this->db->exec( $sql ) )
        {
    ...
    public function taobao_cat_transfer( $data = "" )
    {
        $cat_info = json_decode( $data['data'] );
        foreach ( array(
            $cat_info['rsp']['seller_cats']
        ) as $name => $value )
        {
            $aDta = array(
                "cat_id" => $value['cid'],
                "cat_name" => $value['name'],
                "parent_id" => $value['parent_cid'],
                "p_order" => $value['sort_order']
            );
    ...
    public function api_products_goods_insert( $data )
    {
    ...
    public function import_taobao_database( $data = "" )
    {
        if ( $data )
        {
    ...
    public function insert_pic( &$v_info, &$goods_id )
    {
        if ( $v_info['item_img'] )
        {
    ...
    public function insert_more_specifications( &$v_info, &$goods_id, &$spec, &$spec_desc )
    {
        if ( !is_array( $v_info ) )
        {
            return FALSE;
        ...
        foreach ( ( array )$v_info['sku'] as $k_sku => $v_sku )
        {
            ...
            $pdt_desc = substr( $pdt_desc_tmp_arr, 0, -1 );
            $props = array(
                "spec" => $spec,
                "idata" => array( ),
    ...
    public function single_specifications( &$v_info )
    {
        $rs = $this->db->exec( "select * from sdb_goods where 0=1" );
        $aData = array(
            "type_id" => $v_info['cid'],
    ...
    public function inser_into_taobao_common( &$v_info, &$goods_id )
    {
        $p_value = array(
            "iid" => $v_info['iid'],
            "title" => $v_info['title'],
    ...
    public function taobao_upload_complete( )
    {
        return $this->system->setConf( "system.taobao_uploaded", TRUE );
    }

    public function get_taobao_spec( &$v_info )
    {
        foreach ( ( array )$v_info['sku'] as $k_sku => $v_sku )
        {
            $tp_result = explode( ":", str_replace( ";", ":", $v_sku['properties'] ) );
            $k = 0;
            for ( ; $k < count( $tp_result ) / 2; ++$k )
            {
                ...
                if ( !in_array( $tp_result[$k * 2 + 1], $props_tmparr[$tp_result[$k * 2]] ) )
                {
                    ...
                    {
                        $spec_tmp[$mem_row1['prop_id']] = $mem_row1['prop_name'];
                        $mem_row1['is_color_prop'] == 1 ? ( $spec_type = "image" ) : ( $spec_type = "text" );
                        if ( $v_info['property_alias'] && preg_match( "/".$name."+[\\s\\S]+[\\;|\\z]/", $v_info['property_alias'], $match ) )
                        {
