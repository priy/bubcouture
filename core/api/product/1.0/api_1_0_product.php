...
class api_1_0_product extends shop_api_object
{
    ...
    public function set_products_bn( $data )
    {
        safevar( $data );
    ...
    public function set_product_freeze_store( $data )
    {
    ...
    public function search_products_list( $data )
    {
    ...
    public function set_product_store( $data )
    {
        $sql = "update sdb_products set store=".intval( 0 < $data['store'] ? $data['store'] : 0 )." where bn=\"".$data['product_id']."\" or product_id=".$data['product_id'];
    ...
    public function set_products_store( $data )
    {
        ...
        {
            $sql = "update sdb_products set store='".intval( 0 < $val['store'] ? $val['store'] : 0 )."' where bn='".$val['bn']."'";
            if ( !( $rs = $this->db->query( $sql ) ) )
            {
                $this->api_response( "fail", "db error", $data );
            }
