...
class api_b2b_3_0_product extends shop_api_object
{
    ...
    (
        ...
        (
            ...
            "debug" => "",
            "level" => "error",
            "desc" => "查询了无权限的产品线或者本身无产品线权限"
        )
    );

    public function filt_goods( $data )
    {
            ...
            if ( $goods_info )
            {
                foreach ( $goods_info as $g )
                {
                    ...
                    foreach ( $pline as $v )
                    {
                        ...
                        else if ( $g['brand_id'] == $v['brand_id'] && in_array( $g['cat_id'], $v['cat_id'] ) )
                        {
                            $tmp_flag = TRUE;
                        }
                        if ( $tmp_flag )
                ...
                if ( !empty( $pdt_desc ) )
                {
                    ...
                    foreach ( $product_info as $product )
                    {
                        ...
                        foreach ( $props['spec'] as $spec_id => $spec )
                        {
                            ...
                            $tmp_product['props'][$spec_id]['spec_value'] = array(
                                $spec_value_id => array(
    ...
    public function match_goods( $data )
    {
        safevar( $data );
    ...
    public function _checkDealerPurview( $member = "", $arr_goods )
    {
