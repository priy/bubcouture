...
class notemodel
{
    ...
    public function add( $operation, $getdata = "", $postdata = "", $appids = array( ), $pri = 0 )
    {
        ...
        if ( $insert_id )
        {
            $this->db->query( "REPLACE INTO ".UC_DBTABLEPRE."vars SET name='noteexists', value='1'" );
        }
        return $insert_id;
    }
    ...
    public function _send( )
    {
        ...
        foreach ( ( array )$this->apps as $appid => $app )
        {
            ...
            if ( $app['recvnote'] && $appnotes != 1 && 0 - UC_NOTE_REPEAT < $appnotes )
            {
    ...
    public function _gc( )
    {
        if ( rand( 0, UC_NOTE_GC ) == 0 )
        {
            $this->db->query( "DELETE FROM ".UC_DBTABLEPRE."notelist WHERE closed='1'" );
        }
    }

    public function _close_note( $note, $apps, $returnsucceed )
    {
        ...
        foreach ( $apps as $key => $app )
        {
            ...
            if ( !$app['recvnote'] || $appstatus == 1 || $appstatus <= 0 - UC_NOTE_REPEAT )
            {
                --$appcount;
            }
        }
