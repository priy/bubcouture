...
function uc_fopen2( $url, $limit = 0, $post = "", $cookie = "", $bysocket = FALSE, $ip = "", $timeout = 15, $block = TRUE )
{
    ...
    $data = uc_fopen( $url, $limit, $post, $cookie, $bysocket, $ip, $timeout, $block );
    return $data;
}
...
{
    call_user_func( UC_API_FUNC, "feed", "add", array(
        "icon" => $icon,
        "appid" => UC_APPID,
        "uid" => $uid,
        "username" => $username,
...
function uc_user_allmerge( $data, &$uidGroup )
{
    return call_user_func( UC_API_FUNC, "user", "allmerge", array(
        $data,
        $uidGroup
    ) );
}
...
$GLOBALS['GLOBALS']['uc_controls'] = array( );
?>
