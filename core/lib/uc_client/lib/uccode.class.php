...
class uccode
{
    ...
    {
        $this->uccode = array( "pcodecount" => -1, "codecount" => 0, "codehtml" => "" );
    }
    ...
    public function complie( $message )
    {
        ...
        {
            $message = preg_replace( array( "/\\[img\\]\\s*([^\\[\\<\r\n]+?)\\s*\\[\\/img\\]/ies", "/\\[img=(\\d{1,4})[x|\\,](\\d{1,4})\\]\\s*([^\\[\\<\r\n]+?)\\s*\\[\\/img\\]/ies" ), array( "\$this->bbcodeurl('\\1', '<img src=\"%s\" border=\"0\" onclick=\"zoom(this, this.src)\" onload=\"attachimg(this, \\'load\\')\" alt=\"\" />')", "\$this->bbcodeurl('\\3', '<img width=\"\\1\" height=\"\\2\" src=\"%s\" border=\"0\" alt=\"\" />')" ), $message );
        }
        $i = 0;
        for ( ; $i <= $this->uccode['pcodecount']; ++$i )
        {
    ...
    public function parseurl( $url, $text )
    {
        if ( !$url && preg_match( "/((https?|ftp|gopher|news|telnet|rtsp|mms|callto|bctp|ed2k|thunder|synacast){1}:\\/\\/|www\\.)[^\\[\"']+/i", trim( $text ), $matches ) )
        {
            ...
            {
                $text = substr( $url, 0, intval( $length * 0.5 ) )." ... ".substr( $url, 0 - intval( $length * 0.3 ) );
            }
