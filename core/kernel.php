...
class kernel
{
    ...
    public $__setting = NULL;
    public $_funcPkg = NULL;
    public $models = NULL;
    public $_app_version = "4.8.5";
    public $__call_libs = NULL;
    public $_co_depth = 0;
    public $memcache = FALSE;
    public $_base_link = NULL;
    public $model_dir = "model";

    public function kernel( )
    {
        error_reporting( E_ALL ^ E_NOTICE ^ E_WARNING );
        $GLOBALS['GLOBALS']['system'] =& $this;
        ...
        else
        {
            ...
            if ( isset( $this->__metadata['GTASK_REMINDER'] ) && 0 < $this->__metadata['GTASK_REMINDER'] && $this->__metadata['GTASK_REMINDER'] < time( ) )
            {
    ...
    public function apply_modifiers( &$output, $type )
    {
        if ( !constant( "SAFE_MODE" ) )
        {
            $output_modifiers = $this->getConf( "system.output_modifiers" );
            $modifiers = array_merge( ( array )$output_modifiers['*'], ( array )$output_modifiers[$type.":*"], ( array )$output_modifiers[$type.":".strtolower( $this->request['action']['controller'] ).":*"], ( array )$output_modifiers[$type.":".strtolower( $this->request['action']['controller'].":".$this->request['action']['method'] )] );
            if ( $modifiers )
            {
    ...
    public function getmeta( $key, &$ret )
    {
    ...
    public function savemeta( $key, $value )
    {
        ...
        register_shutdown_function_once( array(
            $this,
            "_final_save_meta"
        ), "savemeta124" );
    }

    public function _final_save_meta( )
    {
    ...
    public function log( $message, $key = "message" )
    {
        error_log( );
    }
    ...
    public function &getLink( $controller, $method, $args = NULL, $extname = NULL )
    {
        if ( $controller == "index" )
        {
            return "";
    ...
    public function api_call( $instance, $host, $file, $port = 80, $tolken )
    {
        ...
        {
            ( $host, $file, $port, $tolken );
            $this->intance_api[$instance] = new api_utility( );
        }
    ...
    public function &loadModel( $modelName, $single = TRUE )
    {
        ...
        {
            require( CORE_INCLUDE_DIR."/modelFactory.php" );
        }
        $plugin_path = explode( "/", $modelName );
        if ( $plugin_path[0] == "plugins" && file_exists( $modleFile = PLUGIN_DIR."/app/".$plugin_path[1]."/mdl.".basename( $modelName ).".php" ) )
        {
            require( $modleFile );
        }
        if ( !class_exists( $className ) && file_exists( $modelFile = CORE_DIR."/".$this->model_dir."/".dirname( $modelName )."/mdl.".basename( $modelName ).".php" ) )
        {
            require( $modelFile );
        }
        if ( defined( "CUSTOM_CORE_DIR" ) )
        {
            ...
            if ( !class_exists( $CusclassName ) && file_exists( $cusinc = CUSTOM_CORE_DIR."/model/".dirname( $modelName )."/cmd.".basename( $modelName ).".php" ) )
            {
                require( $cusinc );
    ...
    public function callAction( &$objCtl, $act_method, $args = NULL )
    {
        ...
        {
            $this->request['action']['ident'] = strtolower( "shop:".$this->request['action']['controller'].":".$this->request['action']['method'] );
        }
    ...
    public function sprintf( )
    {
        ...
        $str = $args[0];
        unset( $args[0] );
        $str = preg_replace_callback( "/\\\$([a-z\\.\\_0-9]+)\\\$/is", array(
            $this,
            "_rep_conf"
        ), $str );
        foreach ( $args as $k => $v )
