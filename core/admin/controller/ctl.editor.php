...
class ctl_editor extends adminPage
{
    ...
    public function find( $type, $keywords )
    {
        if ( !$keywords )
        {
            echo __( "请输入关键字。" );
        ...
        {
            $mod =& $this->system->loadModel( "goods/products" );
            foreach ( $mod->getList( "goods_id,name", array(
                "name" => $keywords
            ) ) as $k => $r )
            {
                $list[] = array(
                    "url" => $this->system->realUrl( "product", "index", array(
                        $r['goods_id']
                    ), "html", $this->system->base_url( ) ),
                    "label" => $r['name']
                );
            }
        ...
        {
            $mod =& $this->system->loadModel( "content/article" );
            foreach ( $mod->getList( "article_id,title", array(
                "keywords" => $keywords
            ) ) as $k => $r )
            {
                $list[] = array(
                    "url" => $this->system->realUrl( "article", "index", array(
                        $r['article_id']
                    ), "html", $this->system->base_url( ) ),
                    "label" => $r['title']
                );
    ...
    public function image( $showpicset = 1 )
    {
    ...
    public function _eH( $errno, $errstr, $errfile, $errline )
    {
        restore_error_handler( );
        echo "<script>window.top.uploadCallback(false)</script>";
    }

    public function gallery( $tag = 0, $page = 1, $file_type = 0 )
    {
        $pubFile =& $this->system->loadModel( "system/pubfile" );
        $p = 18;
        $result = $pubFile->getList( NULL, $filter = array(
            "tag" => $tag,
            "file_type" => $file_type
        ), $p * ( $page - 1 ), $p );
    ...
    public function gallery_SWF( $tag = 0, $page = 1, $file_type = 0 )
    {
        $pubFile =& $this->system->loadModel( "system/pubfile" );
        $p = 18;
        $result = $pubFile->getList( NULL, array(
            "tag" => $tag,
            "file_type" => $file_type
        ), $p * ( $page - 1 ), $p );
        foreach ( $result as $k => $v )
    ...
    public function object_rows( )
    {
        if ( $_POST['data'] )
        {
            ...
            $this->pagedata['_input'] = array(
                "items" => $obj->getList( $obj->idColumn.",".$_POST['cols'], array(
                    $obj->idColumn => $_POST['data']
                ) ),
                "idcol" => $obj->idColumn,
    ...
    {
        $obj = $this->system->loadModel( $_GET['object'] );
        $from = "from";
    ...
    {
        $filter = $_GET['filter'];
        $GLOBALS['_GET']['obj_id'] = substr( md5( $_GET['object'] ), 0, 6 );
        $this->_select_obj( $filter );
    ...
    public function _select_obj( $filter )
    {
        if ( !isset( $filter['marketable'] ) )
        {
            $filter['marketable'] = "true";
        }
        $o =& $this->system->loadModel( $_GET['object'] );
        $limit = 10;
        if ( !$_GET['page'] )
        {
            $GLOBALS['_GET']['page'] = 1;
