...
class ctl_demo_data extends adminPage
{
    ...
    public function install_demo_data( )
    {
        ...
        if ( isset( $_POST['url'] ) )
        {
            ...
            $GLOBALS['_POST'] = array(
                ...
                "succ_url" => "http://".$_SERVER['HTTP_HOST'].dirname( $_SERVER['PHP_SELF'] )."/index.php?ctl=service/demo_data&act=do_install&p[0]=template",
    ...
    public function do_install( $action = FALSE )
    {
        ...
        if ( $action == "template" )
        {
            ...
            if ( $tar->openTAR( $file_path ) )
            {
                foreach ( $tar->files as $id => $file )
                {
                    ...
                    if ( substr( $file['name'], -4 ) == ".sql" )
                    {
                        ...
                        {
                            echo $this->_fetch_compile_include( "service/download_complete_handle.html", array(
                                "info" => "体验数据安装失败".$d_install->error
                            ) );
                            exit( );
                        }
                    }
            ...
            $GLOBALS['_POST'] = array(
                ...
                "succ_url" => "http://".$_SERVER['HTTP_HOST'].dirname( $_SERVER['PHP_SELF'] )."/index.php?ctl=service/demo_data&act=do_install"
            );
            $download->start( );
            exit( );
        }
