...
class ctl_default extends adminPage
{
    ...
    public function index( )
    {
        ...
        if ( !IN_AJAX )
        {
            ...
            if ( constant( "SAAS_MODE" ) )
            {
                ...
                if ( $shopinfo = $saas->native_svc( "host.getinfo", array(
                    "host_id" => HOST_ID
                ) ) )
                {
                    if ( 0 < $shopinfo['response_code'] )
                    {
    ...
    public function _fetchM( $menu, &$arr, $p )
    {
        foreach ( $menu as $m )
        {
            if ( $m['link'] )
            {
                ...
                else
                {
                    $arr[$m['link']] = array(
                        "link" => $m['link'],
                        "path" => ( 0 < count( $p ) ? implode( "/", $p )."/" : "" ).$m['label'],
                        "key" => array( 1 )
                    );
                }
                if ( $m['keywords'] )
    ...
    public function tnode( $model, $id, $depth )
    {
    ...
    public function status( )
    {
        ...
        $this->pagedata['allstatus'] = array(
            "ORDER_NEW" => array(
                "label" => __( "未处理订单" ),
                "url" => "index.php?ctl=order/order&act=index&view=1&filter=".urlencode( serialize( array(
                    "pay_status" => array( "v" => "0", "t" => "未处理" )
                ) ) )
            ),
            "GOODS_ALERT" => array(
                "label" => __( "库存报警" ),
                "url" => "index.php?ctl=goods/product&act=index&filter=".urlencode( serialize( array(
                    "storeless" => array(
                        "v" => $storeless,
                        "t" => "库存小于等于".$storeless
                    )
            ...
            "GNOTIFY" => array(
                "label" => __( "缺货通知" ),
                "url" => "index.php?ctl=goods/product&act=index"
            ),
            "GDISCUSS" => array(
                "label" => __( "商品评论" ),
                "url" => "index.php?ctl=goods/discuss&act=index&filter=".urlencode( serialize( array(
                    "adm_read_status" => array( "v" => "false", "t" => "未阅读" )
                ) ) )
            ),
            "GASK" => array(
                "label" => __( "购买咨询" ),
                "url" => "index.php?ctl=member/gask&act=index"
            ),
            "GOODS_ONLINE" => array(
                "label" => __( "上架商品" ),
                "url" => "index.php?ctl=goods/product&act=index"
            ...
            "ORDER_MESSAGE" => array(
                "label" => __( "新留言订单" ),
                "url" => "index.php?ctl=order/order&act=new_order_message_list"
            )
        );
    ...
    public function _action_finder_colset( $params )
    {
    ...
    public function get_menulist( $searchPanel )
    {
        header( "Content-type:text/html;charset=utf-8" );
    ...
    public function check_api_maintenance( )
    {
    ...
    public function shownewtools( )
    {
        $this->display( "appTaobaoIntro.html" );
    }

    public function getcertidandurl( )
    {
