...
class ctl_delivery_printer extends objectPage
{
    ...
    public $object = "trading/dly_printer";
    public $finder_action_tpl = "order/dly_printer_action.html";
    public $finder_default_cols = "_cmd,prt_tmpl_title,shortcut";
    public $filterUnable = TRUE;

    public function ctl_delivery_printer( )
    {
        parent::objectpage( );
        ...
        if ( $goodsItems )
        {
            ...
            foreach ( $goodsItems as $k => $item )
            {
                if ( $item['is_type'] == "goods" )
                {
                    ...
                    $data['order_name'][$i] = array(
                        "name" => $aProduct['name']
                    );
                    $data['order_name_a'][$i] = array(
                        "name" => $aProduct['name'],
                        "num" => $item['nums']
                    );
                    $data['order_name_as'][$i] = array(
                        "name" => $aProduct['name'],
                        "num" => $item['nums'],
                        "spec" => $aProduct['pdt_desc']
                    );
                    $data['order_name_ab'][$i] = array(
                        "name" => $aProduct['name'],
                        "num" => $item['nums'],
                        "bn" => $aProduct['bn']
                    );
    ...
    public function do_upload_pkg( )
    {
        ...
        if ( $extname == ".dtp" )
        {
            if ( $tar->openTAR( $file['tmp_name'] ) && $tar->containsFile( "info" ) )
            {
                ...
                if ( ( $tpl_id = $this->model->insert( $info ) ) && $tar->containsFile( "background.jpg" ) )
                {
    ...
    {
        $this->pagedata['tmpl'] = array( "prt_tmpl_title" => "", "prt_tmpl_width" => 240, "prt_tmpl_height" => 135 );
        if ( PRINTER_FONTS )
        {
            $this->pagedata['font'] = explode( "|", PRINTER_FONTS );
        }
    ...
    public function save( )
    {
        ...
        {
            if ( $this->model->update( $_POST, array(
                "prt_tmpl_id" => $_POST['prt_tmpl_id']
            ) ) )
            {
        ...
        $GLOBALS['_POST']['tmp_bg'] = trim( $_POST['tmp_bg'] );
    ...
    public function download( $tmpl_id )
    {
    ...
    public function upload_bg( $printer_id )
    {
