...
class ctl_passport extends adminPage
{
    ...
    public function login( )
    {
        ...
        if ( $_GET['return'] )
        {
            $this->pagedata['return'] = $_GET['return'];
        }
        $this->display( "login.html" );
    }

    public function checkVeryCode( )
    {
        if ( $this->system->getConf( "system.admin_verycode" ) || $this->login_times_error < $this->system->getConf( "system.admin_error_login_times" ) && time( ) < intval( $this->system->getConf( "system.admin_error_login_time" ) + 3600 ) )
        {
            return TRUE;
        }
        else
    ...
    public function certi_validate( )
    {
        ...
        {
            $return = array( "res" => "succ", "msg" => "", "info" => "" );
            echo json_encode( $return );
        }
        else
        {
            $return = array( "res" => "fail", "msg" => "000001", "info" => "You have the different session!" );
            echo json_encode( $return );
        }
    }
