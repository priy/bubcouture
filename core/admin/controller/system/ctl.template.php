...
class ctl_template extends adminPage
{
    ...
    public function setColor( $theme, $color )
    {
        if ( $theme && $color )
        {
    ...
    public function tempalte_rename( $old_name, $new_name, $theme )
    {
        $o = $this->system->loadModel( "system/template" );
    ...
    public function copy_tpl( $theme, $tpl, $type )
    {
    ...
    {
        if ( isset( $_POST['url'], $_POST['tpl_name'] ) && isset( $_POST['fullsize'] ) )
        {
            ...
            $GLOBALS['_POST'] = array(
                ...
                "succ_url" => "http://".$_SERVER['HTTP_HOST'].dirname( $_SERVER['PHP_SELF'] )."/index.php?ctl=system/template&act=install"
            );
            $download->start( );
        }
        exit( );
    ...
    public function reset( $theme, $xml = "" )
    {
        $o = $this->system->loadModel( "system/template" );
        $o->reset( $theme, $xml );
    ...
    public function dlPkg( $theme )
    {
        $o = $this->system->loadModel( "system/template" );
        $o->outputPkg( $theme );
    }

    public function previewImg( $theme, $pic = "preview.jpg" )
    {
        $o = $this->system->loadModel( "system/template" );
    ...
    {
        $o = $this->system->loadModel( "system/template" );
        $o->editHtml( $this->in['title'], array(
            "page_name" => $this->in['name'],
            "page_content" => $this->in['html'],
            "page_time" => time( )
        ) );
    }
    ...
    public function setTemplateDefault( $theme, $type, $tpl )
    {
        $this->system->setConf( "system.custom_template_".$type, $tpl );
        $this->splash( "success", "index.php?ctl=system/template&act=edit&p[0]=".$theme );
    }

    public function saveConfig( $theme )
    {
        $o = $this->system->loadModel( "system/template" );
        $info = $o->getThemeInfo( $theme );
    ...
    public function widgetsSet( $theme, $view )
    {
        header( "Content-Type: text/html; charset=utf-8" );
        $this->path[] = array(
            "text" => __( "模板可视化编辑" )
            ...
            foreach ( $_POST['widgets'] as $widgets_id => $base )
            {
                ...
                if ( $_POST['html'][$widgets_id] )
                {
                    $widgetsSet[$widgets_id] = array(
                        ...
                        "border" => "__none__",
                        "params" => array(
                            "html" => stripslashes( $_POST['html'][$widgets_id] )
                        )
                    );
                }
                else
    ...
    public function saveWg( $widgets_type, $widgets_id, $theme, $domid )
    {
        header( "Content-Type: text/html;charset=utf-8" );
        unsafevar( $_POST );
        ...
        else if ( preg_match( "/^tmp_([0-9]+)\$/i", $widgets_id, $match ) )
        {
            $_SESSION['_tmp_wg'][$match[1]] = $set;
        }
        echo $widgets->adminWgBorder( array(
            "title" => $set['title'],
            "widgets_id" => $widgets_id,
            "domid" => $set['domid'],
            "border" => $set['border'],
            "widgets_type" => $set['widgets_type'],
            "html" => $widgets->fetch( $set, TRUE, $widgets_id ),
            "border" => $set['border']
        ), $theme, TRUE );
    }

    public function editWidgets( $widgets_id, $theme )
    {
        $widgets = $this->system->loadModel( "content/widgets" );
        if ( is_numeric( $widgets_id ) )
        {
        ...
        $this->pagedata['widgetsTpl'] = str_replace( "'", "\\'", $widgets->adminWgBorder( array(
    ...
    public function doAddWidgets( $widgets_type, $theme )
    {
        error_reporting( E_ERROR | E_WARNING | E_PARSE );
        $widgets = $this->system->loadModel( "content/widgets" );
    ...
    public function addWidgetsPage( $themes )
    {
        $widgets = $this->system->loadModel( "content/widgets" );
    ...
    public function getWidgetsInfo( $type = "" )
    {
        if ( $_GET['widgets'] )
        {
            $widgets = $this->system->loadModel( "content/widgets" );
    ...
    public function addWidgetsPageExtend( $themes, $type = "" )
    {
        $widgets = $this->system->loadModel( "content/widgets" );
        $this->pagedata['themes'] = $themes;
    ...
    public function insertWg( $widgets_type, $domid, $theme )
    {
        header( "Content-Type: text/html;charset=utf-8" );
        error_reporting( E_ERROR );
        unsafevar( $_POST );
        ...
        $data = $widgets->adminWgBorder( array(
            "title" => $set['title'],
            "domid" => $set['domid'],
            "border" => $set['border'],
            "widgets_type" => $set['widgets_type'],
            "html" => $widgets->fetch( $set, TRUE ),
            "border" => $set['border']
        ), $theme, TRUE );
        echo $data;
    }

    public function copyWg( $domid, $widgetid )
    {
        $widgets = $this->system->loadModel( "content/widgets" );
    ...
    public function editor( $theme, $file, $template = NULL, $file_name = NULL )
    {
        $this->path[] = array(
            "text" => __( "模板源码编辑" )
        );
    ...
    public function removePage( $theme, $file )
    {
    ...
    public function recoverSource( $theme, $bakfile, $file )
    {
    ...
    {
        if ( strtolower( substr( $_POST['file'], -5 ) ) != ".html" )
        {
            $GLOBALS['_POST']['file'] = "index.html";
        }
    ...
    public function templetePreview( $tpl, $file )
    {
        header( "Content-Type: text/html; charset=utf-8" );
        $this->system->__session_close( );
        ...
        $smarty->theme = $tpl;
        $this->theme = $tpl;
        $smarty->register_prefilter( array(
            $this,
            "_prefix_tpl"
        ) );
        $smarty->_plugins['compiler']['main'] = array(
            $this,
            "_main"
        );
        $smarty->_plugins['function']['link'] = array(
            $this,
            "mkUrl"
        );
        $smarty->_plugins['function']['footer'] = array(
            $this,
            "_footer"
        );
        $smarty->_plugins['function']['header'] = array(
            $this,
            "_header"
        );
        $smarty->_plugins['resource']['user'] = array(
            array(
                $this,
                "_get_template"
            ),
    ...
    public function _require( $tag_args, &$smarty )
    {
        $attrs = $tag_args;
        $output = "";
        if ( isset( $assign_var ) )
    ...
    public function _get_secure( )
    {
        return TRUE;
    }

    public function _get_trusted( )
    {
        return TRUE;
    }

    public function _get_template( $tpl_name, &$tpl_source, &$smarty )
    {
    ...
    public function _get_timestamp( $tpl_name, &$tpl_timestamp, &$smarty )
    {
    ...
    public function _main( $tag_args, &$smarty )
    {
        return "?><div class=\"system-widgets-box\">&nbsp;</div><?php";
    }

    public function _prefix_tpl( $tpl, &$smarty )
    {
