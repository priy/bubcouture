...
class ctl_appmgr extends adminPage
{
    ...
    public function index( )
    {
        ...
        foreach ( $apps as $k => $app )
        {
            ...
            {
                $aTmp[] = array(
                    "app_key" => $app['plugin_ident'],
                    "version" => $app['plugin_version']
                );
            }
            $apps[$k] = $app;
        }
    ...
    public function download_update_app( )
    {
        if ( isset( $_GET['url'] ) )
        {
            ...
            $GLOBALS['_POST'] = array(
                ...
                "succ_url" => "http://".$_SERVER['HTTP_HOST'].dirname( $_SERVER['PHP_SELF'] )."/index.php?ctl=system/appmgr&act=do_update&app_status=".$_GET['app_status']
            );
            $download->start( );
        }
        exit( );
    ...
    public function view( $ident )
    {
        $appmgr = $this->system->loadModel( "system/appmgr" );
        $this->pagedata['app'] = $appmgr->info( $ident );
        $this->pagedata['propmap'] = array( "ver" => "版本", "website" => "网址", "author" => "作者", "baseurl" => "访问路径", "plugin_path" => "文件路径" );
    ...
    public function install_online( )
    {
        if ( isset( $_GET['url'] ) )
        {
            ...
            $GLOBALS['_POST'] = array(
                ...
                "succ_url" => "http://".$_SERVER['HTTP_HOST'].dirname( $_SERVER['PHP_SELF'] )."/index.php?ctl=system/appmgr&act=do_install_online"
            );
            $download->start( );
        }
        exit( );
    ...
    public function install( $ident, $type = "offline", $is_update = FALSE )
    {
        $appmgr = $this->system->loadModel( "system/appmgr" );
        $app =& $appmgr->load( $ident );
        if ( $app->depend && ( $err_str = $appmgr->getappByident( $app->depend ) ) )
        {
        ...
        foreach ( $app->listener( ) as $eventOrg => $handle )
        {
            ...
            else
            {
                ...
                if ( method_exists( $mdl, "events" ) && ( $events = $mdl->events( ) && $events[$event] ) )
                {
    ...
    public function do_uninstall( $ident )
    {
        $appmgr = $this->system->loadModel( "system/appmgr" );
        $this->begin( "index.php?ctl=system/appmgr&act=index" );
    ...
    public function setting_save( $ident )
    {
        $appmgr = $this->system->loadModel( "system/appmgr" );
        $app =& $appmgr->load( $ident );
    ...
    public function dofeedback( $app_id )
    {
        $certi = $this->system->loadModel( "service/certificate" );
        $certi_id = $certi->getCerti( );
    ...
    {
        $return = $this->sendRequest( "app.get_appinfo", array(
            "app_key" => $_GET['app_ident'],
    ...
    public function appkey_count( )
    {
        ...
        $certi = $this->system->loadModel( "service/certificate" );
        $certi_id = $certi->getCerti( );
        $this->sendRequest( "app.count_appkey", array(
            "certi_id" => $certi_id,
    ...
    public function appupdate_count( )
    {
        $cet_ping = ping_url( "http://esb.shopex.cn/api.php" );
        if ( !strstr( $cet_ping, "HTTP/1.1 200 OK" ) )
        {
        ...
        $certi = $this->system->loadModel( "service/certificate" );
        $certi_id = $certi->getCerti( );
        $this->sendRequest( "app.count_appupdate", array(
            "certi_id" => $certi_id,
            "app_key" => $_POST['app_key'],
            "version" => $_POST['version']
        ) );
    ...
    public function getAppUpdateTime( $aData )
    {
        $cet_ping = ping_url( "http://esb.shopex.cn/api.php" );
        if ( !strstr( $cet_ping, "HTTP/1.1 200 OK" ) )
        {
        ...
        $return = $this->sendRequest( "app.get_appkey_time", array(
    ...
    public function sendRequest( $service, $aData )
    {
