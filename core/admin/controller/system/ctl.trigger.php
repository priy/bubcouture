...
class ctl_trigger extends objectPage
{
    ...
    public $name = "";
    public $object = "system/trigger";
    public $finder_action_tpl = "system/trigger/finder_action.html";
    public $finder_default_cols = "_cmd,filter_str,action_str,trigger_event,trigger_memo,active";
    ...
    public function edit( $trigger_id )
    {
        ...
        list(  ) = explode( ":", $trigger['trigger_event'] );
        $trigger['event'] = $Var_264;
        list( $target ) = explode( ":", $trigger['trigger_event'] );
    ...
    public function addNew( $target = "system" )
    {
        $this->path[] = array(
            "text" => __( "新建" ).$this->pagedata['target_name'].__( "网店机器人" )
        );
        $this->pagedata['actions'] = $this->_all_actions( $target );
        $this->pagedata['target'] = $target;
        $this->pagedata['target_name'] = $this->model->trigger_points[$target];
        if ( $target == "system" )
        {
            ...
            $this->pagedata['trigger']['trigger_define']['actions'] = array(
                array(
                    "act" => key( $first_action_grp )
                )
            );
    ...
    public function _all_actions( $target )
    {
    ...
    public function showAction( $func )
    {
        $addons =& $this->system->loadModel( "system/addons" );
    ...
    public function showfilter( $target_event )
    {
    ...
    public function _init_filter( $target, $event = NULL )
    {
    ...
    public function save( )
    {
        ...
        if ( $_POST['trigger_id'] )
        {
            $status = $this->model->update( $trigger, array(
