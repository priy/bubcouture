...
class ctl_setting extends adminPage
{
    ...
    public function guide_status( $getable = FALSE )
    {
        if ( $getable )
        {
    ...
    public function siteBasicEdit( )
    {
        ...
        if ( $_FILES )
        {
            foreach ( $_FILES['setting']['name'] as $k => $name )
            {
                ...
                $GLOBALS['_POST']['setting'][$k] = $storager->save_upload( $file, "default", "logo" );
                if ( !$_POST['setting'][$k] )
                {
    ...
    public function shoppingBasicEdit( )
    {
        $this->begin( "index.php?ctl=system/setting&act=shoppingbasic" );
        $GLOBALS['_POST']['setting']['site.tax_ratio'] = $_POST['setting']['site.tax_ratio'] / 100;
        $GLOBALS['_POST']['setting']['site.market_rate'] = $_POST['setting']['site.market_rate'] ? $_POST['setting']['site.market_rate'] : 0;
        $this->end( $this->settingEdit( ), __( "修改成功" ) );
    }
    ...
    public function watermark( )
    {
        ...
        if ( $loaded )
        {
            ...
            $this->pagedata['enable_options'] = array(
                "0" => __( "关闭" ),
                "1" => __( "开启图片水印" ),
                "2" => __( "开启文字水印" )
            );
            $this->pagedata['wm_pos_options'] = array(
                "0" => __( "居中" ),
                "1" => __( "顶部居左" ),
                "2" => __( "顶部居右" ),
                "3" => __( "底部居右" ),
    ...
    public function watermarkEdit( )
    {
        $this->begin( "index.php?ctl=system/setting&act=watermark" );
        $data =& $_POST;
        $storager =& $this->system->loadModel( "system/storager" );
        $files = array( );
        $procssing_files = array( "default_thumbnail_pic" => "site.default_thumbnail_pic", "default_big_pic" => "site.default_big_pic", "default_small_pic" => "site.default_small_pic", "wm_small_pic" => "site.watermark.wm_small_pic", "wm_big_pic" => "site.watermark.wm_big_pic", "spec_default_pic" => "spec.default.pic" );
        ...
        if ( $loaded )
        {
            ...
            if ( $watermark['wm_small_enable'] )
            {
                ...
                else if ( $watermark['wm_small_enable'] == 2 )
                {
            ...
            if ( $watermark['wm_big_enable'] )
            {
                ...
                else if ( $watermark['wm_big_enable'] == 2 )
                {
    ...
    public function watermarkPreview( $tag )
    {
        $ib =& $this->system->loadModel( "utility/magickwand" );
        if ( $ib->magickwand_loaded )
        {
    ...
    public function createPreviewImg( $tag, $upload = FALSE )
    {
        header( "Content-type: text/html;charset=utf-8" );
        if ( $upload )
        {
        ...
        else
        {
            ...
            $ib->wm_image_name = $_FILES["wm_".$tag."_pic"]['tmp_name'] ? $_FILES["wm_".$tag."_pic"]['tmp_name'] : $storager->getFile( $this->system->getConf( "site.watermark.wm_".$tag."_pic" ) );
            $ib->wm_image_transition = $_POST["wm_".$tag."_transition"];
            $ib->wm_text = $_POST["wm_".$tag."_text"];
            $ib->wm_text_size = $_POST["wm_".$tag."_font_size"];
            $ib->wm_text_font = $_POST["wm_".$tag."_font"];
            $ib->wm_text_color = $_POST["wm_".$tag."_font_color"];
            $ib->wm_image_pos = $_POST["wm_".$tag."_loc"];
            $enable = $_POST["wm_".$tag."_enable"];
            $height = $_POST[$tag."_pic_height"];
            $width = $_POST[$tag."_pic_width"];
            switch ( $enable )
            {
            case 0 :
                header( "Content-Type: text/html; charset=utf-8" );
    ...
    public function save_siteinfor( )
    {
        $this->begin( "index.php?ctl=system/setting&act=siteBasic" );
        if ( $_FILES )
        {
    ...
    public function centerSend( $info, $params )
    {
    ...
    public function sendUpdateinfo( $setting_info )
    {
