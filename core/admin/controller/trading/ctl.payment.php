...
class ctl_payment extends objectPage {
    ...
    function _getHtmlString($key,&$val,$rs=array(),&$eventScripts){
        ...
        switch($val['type']){
            ...
            case 'select':
                ...
                $aTemp=array('labelName'=>$val['label'],'params'=>array('type'=>'select','name'=>$key,'value'=>$selOption,'options'=>$tOptions));
                break;
            case 'number':
                $aTemp=array('labelName'=>$val['label'],'params'=>array('type'=>'text','name'=>$key,'value'=>$rs[$key]?$rs[$key]:''));
                break;
            case 'file':
                $aTemp=array('labelName'=>$val['label'],'params'=>array('type'=>'file','name'=>$key,'value'=>$selOption,'options'=>$tOptions));
                break;
            case "radio":
                foreach($val['options'] as $k => $v){
                    $checked="";
    ...
    * @return void
    */
    function editPayment($payId,$payName,$paytype){
    ...
    */
    ...
    function _getPayOpt($sType, $sPayName='', $nFee='', $config='',$fetch=0){
        ...
        $this->pagedata['fee'] = array($nFee,$aTemp['fee']);
