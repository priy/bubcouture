...
class ctl_articles extends objectPage
{
    ...
    public $object = "content/article";
    public $finder_action_tpl = "content/article/finder_action.html";
    public $finder_filter_tpl = "content/article/finder_filter.html";
    ...
    public function addArticle( $node_id )
    {
        if ( $_POST['ifpub'] )
        {
        ...
        if ( $_POST['goodslink'] )
        {
            $GLOBALS['_POST']['goodslink'] = 1;
        }
        else
        {
            $GLOBALS['_POST']['goodslink'] = 0;
        }
        if ( $_POST['hotlink'] )
        {
            $GLOBALS['_POST']['hotlink'] = 1;
        }
        else
        {
            $GLOBALS['_POST']['hotlink'] = 0;
        }
        $oArticle =& $this->system->loadModel( "content/article" );
    ...
    public function _detail( )
    {
        return array(
            "show_detail" => array(
                "label" => __( "文章详细信息" ),
                "tpl" => "content/article/article.html"
            )
        );
    }

    public function show_detail( $nConId )
    {
        ...
        if ( is_array( $goodsinfo ) && 0 < count( $goodsinfo ) )
        {
    ...
    public function addNew( $node_id )
    {
        $this->path[] = array(
            "text" => __( "添加文章" )
        );
    ...
    public function edit( $article_id, $node_id )
    {
        $this->path[] = array(
            "text" => __( "编辑文章" )
        );
    ...
    public function save( $article_id, $node_id )
    {
        if ( $_POST['ifpub'] )
        {
            $GLOBALS['_POST']['ifpub'] = 1;
    ...
    public function getGoods( $num, $goodsId )
    {
