...
class ctl_operator extends objectPage
{
    ...
    public $finder_action_tpl = "admin/finder_action.html";
    public $finder_filter_tpl = "admin/finder_filter.html";
    public $finder_default_cols = "_cmd,username,name,lastlogin,department";
    public $object = "admin/operator";
    public $finderVar = "OperatorMgr";
    public $noRecycle = TRUE;
    public $filterUnable = TRUE;

    public function ctl_operator( )
    {
        parent::objectpage( );
    ...
    public function edit( $nOpId )
    {
        ...
        $operator['select_super'] = array(
            "0" => __( "普通管理员" ),
            "1" => __( "超级管理员" )
        );
        $operator['select_status'] = array(
            "1" => __( "启用" ),
            "0" => __( "禁用" )
        );
    ...
    public function save( )
    {
        ...
        $GLOBALS['_POST']['roles'] = $_POST['roles'] ? $_POST['roles'] : array( );
    ...
    public function delete( )
    {
        if ( $_POST['op_id'] )
        {
            foreach ( $_POST['op_id'] as $k => $v )
            {
                ...
                unset( $this->op_id[$k] );
                if ( !( $v == $this->system->op_id ) && !( count( $_POST['op_id'] ) == 0 ) )
                {
                    echo __( "操作失败" );
                    return;
                }
