...
class ctl_product extends objectPage
{
    ...
    public $object = "goods/products";
    public $allowImport = TRUE;
    public $ioType = "goods";
    public $finder_default_cols = "_cmd,bn,name,cat_id,price,store,marketable,brand_id,weight,d_order,type_id,mktprice";
    public $finder_action_tpl = "product/finder_action.html";
    public $finder_filter_tpl = "product/finder_filter.html";
    public $simpleGoodsId = 1;

    public function goodsAlert( )
    {
    ...
    public function save_cell_value( $id, $key )
    {
        if ( $key == "bn" )
        {
            ...
            $gnum = $oGoods->checkGoodsValid( $d = array(
                "bn" => $_POST['data'],
                "goods_id" => $id
            ) );
            if ( 0 < $gnum )
    ...
    public function show_detail( $gid )
    {
        ...
        $this->pagedata['url'] = $this->system->realUrl( "product", "index", array(
            $gid
        ), NULL );
    ...
    public function _linechart( $chardata )
    {
        ...
        foreach ( $chardata as $d => $v )
        {
            ...
            if ( $lastMonth != ( $month = date( "Y.m", $d ) ) )
            {
                ...
                if ( 0 < $i )
                {
                    $xmark[] = "V,".$color_1.",0,".$i.",1";
                }
            }
            ++$i;
            $xday[] = date( "d", $d );
        }
        return "chs=".$w."x100&chd=t:".implode( ",", $chardata )."&chxt=x,y,x,r&chco=224499&chxl=0:|".implode( "|", $xday )."|1:|0|".round( $max / 2 )."|".$max."|2:|".implode( "|", $xmonth )."|3:|0|".round( $max / 2 )."|".$max."&cht=lc&chds=0,".$max."&chxp=2,".implode( ",", array_keys( $xmonth ) )."&chxs=2,".$color_1.",13&chm=".implode( "|", $xmark )."&chg=5,25,1";
    }

    public function showfilter( $type_id )
    {
        $obj =& $this->system->loadModel( "goods/products" );
        $this->pagedata['filter'] = $obj->getFilterByTypeId( array(
            "type_id" => $type_id
        ) );
    ...
    public function nospec( $cat_id = 0 )
    {
    ...
    {
        $this->pagedata['items'] =& $GLOBALS['_POST']['scheduled'];
    ...
    public function addNew( $cat_id = NULL, $gid = NULL, $type_id = NULL, $brand_id = NULL )
    {
    ...
    public function edit( $goods_id )
    {
        ...
        else
        {
            ...
            {
                if ( $goods['autoset'][$levelid] == 1 )
                {
                    $goods['mprice'][$levelid] = "";
                }
            }
        }
    ...
    public function _editor( $type_id )
    {
        $cat =& $this->system->loadModel( "goods/productCat" );
        ...
        if ( $type_id )
        {
            ...
            if ( is_array( $prototype['setting']['data'] ) )
            {
                ...
                {
                    if ( !( $v['type'] == "select" && is_string( $v['options'] ) ) && !method_exists( $typeObj, $v['options'] ) )
                    {
                        $method = $v['options'];
    ...
    public function clone_goods_img( )
    {
    ...
    public function update( )
    {
        ...
        if ( !empty( $_POST['vars'] ) )
        {
            if ( $oGType->specTransformState == "different" || $oGType->specTransformState == "same" )
            {
                ...
                {
                    $GLOBALS['_POST']['specname'][$dk]['spec_name'] = $dv;
                }
            }
            else
            {
                foreach ( $_POST['vars'] as $dk => $dv )
                {
                    $GLOBALS['_POST']['specname'][$oGType->specTransformHash[$dk]['spec_id']]['spec_name'] = $dv;
                }
            }
        }
        $this->pagedata['goods'] =& $goods;
    ...
    public function toAdd( )
    {
        ...
        if ( is_array( $_POST['bn'] ) )
        {
            ...
            {
                $GLOBALS['_POST']['bn'][$aTmpBnk] = trim( $aTmpBn );
            }
        }
            ...
            foreach ( $_POST['price'] as $k => $price )
            {
                ...
                if ( !$_POST['mktprice'][$k] )
                {
                    ...
                    if ( $this->system->getConf( "site.show_mark_price" ) )
                    {
                        ...
                        {
                            $GLOBALS['_POST']['mktprice'][$k] = $this->system->getConf( "site.market_rate" ) * $oMath->getOperationNumber( $price );
                        }
        ...
        if ( !empty( $data['product_bn'] ) && $objGoods->checkProductBn( $data['product_bn'], $data['goods_id'] ) )
        {
            trigger_error( __( "您所填写的货号已被使用，请检查！" ), E_USER_ERROR );
            $this->end( FALSE, __( "您所填写的货号已被使用，请检查！" ) );
            exit( );
        }
        if ( count( array_unique( $aBn ) ) < count( $aBn ) )
        {
            trigger_error( __( "您所填写的货号已被使用，请检查！" ), E_USER_ERROR );
            $this->end( FALSE, __( "您所填写的货号已被使用，请检查！" ) );
            exit( );
        ...
        {
            $aLink[] = array(
                "goods_1" => $data['goods_id'],
                "goods_2" => $id,
                "manual" => $_POST['linktype'][$id],
                "rate" => 100
            );
        }
        $objProduct =& $this->system->loadModel( "goods/products" );
        ...
        if ( $_POST['supplier_id'] )
        {
            ...
            if ( isset( $_POST['bn'] ) && is_array( $_POST['bn'] ) )
            {
                foreach ( $_POST['bn'] as $nbnk => $nbnv )
                {
                    ...
                    unset( $this->source_bn[$_POST['old_bn'][$nbnk]] );
                }
            }
            else
            {
        ...
        if ( isset( $_POST['commandType'] ) && in_array( $_POST['commandType'], array( "4", "5", "6" ) ) )
        {
    ...
    public function sendNotify( $goods_id )
    {
        ...
        {
            $messenger->actionSend( "goods-notify", array(
                "email" => $notify['email']
            ), $notify['member_id'], $notify );
        }
    ...
    public function mprice( )
    {
        ...
        {
            $level['dis_count'] = 0 < $level['dis_count'] ? $level['dis_count'] : 1;
            $this->pagedata['mPrice'][$level['member_lv_id']] = array(
                "name" => $level['name'],
                "default" => $level['dis_count'] * $_POST['price'],
                "price" => $_POST['level'][$level['member_lv_id']]
            );
        }
    ...
    public function ratelist( )
    {
        ...
        if ( 0 < count( $glist ) )
        {
            $this->pagedata['goods']['rates'] = $objGoods->getList( "name,goods_id,image_default,100 as rate", array(
                "goods_id" => $glist
            ), "name" );
    ...
    public function editItem( $goods_id, $feild, $val )
    {
        $objGoods =& $this->system->loadModel( "trading/goods" );
    ...
    public function view_src_img( $gimage_id )
    {
        $gimage =& $this->system->loadModel( "goods/gimage" );
        $gimage->display_source_by_id( $gimage_id );
    }

    public function newPic( $gid, $type )
    {
        if ( $type == "add" )
        {
    ...
    public function updateSupplierData( $goods_id )
    {
        $g = $this->system->loadModel( "goods/products" );
        if ( $goods = $g->getList( "*", array(
            "goods_id" => $goods_id
        ) ) )
        {
    ...
    public function singleBatchEdit( $editType = "" )
    {
        ...
        if ( $_POST['filter'] )
        {
            $GLOBALS['_POST']['_finder'] = unserialize( $_POST['filter'] );
    ...
    public function saveBatchEdit( )
    {
        ...
        case "brand" :
            ...
            $oPro->batchUpdateArray( $filter['goods_id'], "sdb_goods", array( "brand_id", "brand" ), array(
    ...
    public function nextImage( $same_file_name = FALSE )
    {
        $filter = $_POST;
        $goods =& $this->system->loadModel( "goods/products" );
        ...
        $GLOBALS['_POST']['present_id'] = $_POST['present_id'] + 1;
        usleep( 20 );
    ...
    public function typeTurn( $turnType )
    {
        if ( $_POST['filter'] )
        {
    ...
    public function toTurnType( )
    {
        $GLOBALS['_POST']['_filter'] = unserialize( stripslashes( $_POST['_filter'] ) );
        $oPro = $this->system->loadModel( "goods/products" );
