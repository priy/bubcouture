...
class ctl_spec extends adminPage
{
    ...
    public function addCol( )
    {
        ...
        if ( $_POST['goods'] )
        {
            $this->pagedata['goods_args'] = json_encode( array(
                "goods_args" => $_POST['goods']
            ) );
        }
    ...
    public function specValue( $specId )
    {
        $objSpec =& $this->system->loadModel( "goods/specification" );
    ...
    public function _doCreatePro( $pro, $spec, $goods_args )
    {
        if ( empty( $spec ) )
        {
            $res = array( );
    ...
    {
        $GLOBALS['_POST'] = stripslashes_array( $_POST );
    ...
    public function addRow( )
    {
        if ( $_POST['bn'] )
        {
            if ( !empty( $_POST['bn'][0] ) )
            {
                $GLOBALS['_POST']['bn'][0] = substr( $_POST['bn'][0], 0, -1 ).( substr( $_POST['bn'][0], -1 ) + 1 );
            }
            $GLOBALS['_POST']['goods_args'] = array(
                "price" => $_POST['price'][0],
                "cost" => $_POST['cost'][0],
            ...
            {
                $GLOBALS['_POST']['goods_args']['mprice'][$mpk] = $mpv[0];
            }
        }
    ...
    public function addSpec( $typeId = 0 )
    {
        $objSpec =& $this->system->loadModel( "goods/specification" );
        $aSpec = array( );
        if ( $typeId )
