...
class ctl_level extends objectPage
{
    ...
    public $finder_action_tpl = "member/level/finder_action.html";
    public $object = "member/level";
    ...
    public function _detail( )
    {
        return array(
            "show_detail" => array(
                "label" => __( "会员等级详细" ),
                "tpl" => "member/level/level_edit.html"
            )
        );
    }

    public function showNewMemLv( )
    {
        $this->path[] = array(
            "text" => __( "添加会员等级" )
        );
    ...
    public function addMemLevel( )
    {
        ...
        if ( $switch )
        {
            $GLOBALS['_POST']['experience'] = intval( $_POST['experience'] );
        }
        else
        {
            $GLOBALS['_POST']['point'] = intval( $_POST['point'] );
        }
    ...
    public function show_detail( $nLvId )
    {
        $this->path[] = array(
            "text" => __( "会员等级编辑" )
        );
        ...
        $this->pagedata['lv_type'] = array(
            "retail" => __( "普通零售会员等级" ),
            "wholesale" => __( "批发代理会员等级" )
        );
    }

    public function saveLevelInfo( )
    {
        $oMem =& $this->system->loadModel( "member/level" );
        $GLOBALS['_POST']['dis_count'] = $_POST['dis_count'] / 100;
        ...
        foreach ( $tmpdate as $k => $v )
        {
            if ( !( $_POST['member_lv_id'] != $v['member_lv_id'] ) && !( $_POST['lv_type'] != "wholesale" ) )
            {
                if ( $switch )
                {
                    if ( $_POST['experience'] == $v['experience'] )
