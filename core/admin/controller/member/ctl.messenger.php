...
class ctl_messenger extends adminPage
{
    ...
    public function toRemove( $sd_id, $sender )
    {
    ...
    public function outbox( $sender, $current = 1 )
    {
        $this->path[] = array(
            "text" => __( "发件箱" )
        );
    ...
    public function send( $sender )
    {
        ...
        if ( $systmpl->set( $tmpl_name, $_POST['content'] ) )
        {
            if ( $_POST['targets'] )
            {
                ...
                foreach ( $member->getList( $column, array(
                    "member_id" => array_keys( $_POST['targets'] )
                ), 0, -1 ) as $info )
                {
            ...
            else if ( $_POST['filter'] )
            {
                ...
                foreach ( $member->getList( $column, $filter, $offset, $step ) as $info )
                {
                    ...
                    else if ( ( $custom = $info['custom'] ) && ( $custom = unserialize( $custom ) ) )
                    {
    ...
    public function queue( $sender, $current = 1 )
    {
        $this->path[] = array(
            "text" => __( "待发队列" )
        );
    ...
    public function del( $queue_id, $method )
    {
        if ( $queue_id )
        {
    ...
    public function pagination( $current, $totalPage, $param = "queue" )
    {
