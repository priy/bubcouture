...
class ctl_message extends shopPage
{
    ...
    public function index( $nPage = 1 )
    {
        ...
        $this->pagedata['pager'] = array(
            "current" => $nPage,
            "total" => $aData['page'],
            "link" => $this->system->mkUrl( "message", "index", array(
                $tmp = time( )
            ) ),
            "token" => $tmp
        );
    ...
    public function sendMsgToOpt( )
    {
        ...
        {
            $GLOBALS['_POST'][$key] = strip_tags( $val );
        }
        if ( $this->system->getConf( "comment.verifyCode.msg" ) == "on" && md5( trim( $_POST['verifyCode'] ) ) != $_COOKIE['RANDOM_CODE'] )
        {
