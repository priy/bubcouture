...
class ctl_gift extends shopPage
{
    ...
    public $customer_template_type = "gift";

    public function showTypeList( $catId = 0 )
    {
    ...
    public function showList( $catId = 0, $page = 1 )
    {
        if ( $catId )
        {
            $filter['gid'] = $catId;
        ...
        $this->pagedata['pager'] = array(
            ...
            "link" => $this->system->mkUrl( "gift", "showList", array(
                $catId,
                $tmp = time( )
            ) ),
            "token" => $tmp
    ...
    public function index( $giftId )
    {
        ...
        if ( !$aGift['limit_level'] )
        {
            $aGift['limit_level'] = array( 0 );
        }
