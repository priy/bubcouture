...
class ctl_comment extends shopPage
{
    ...
    public function ctl_comment( &$system )
    {
        parent::shoppage( $system );
        $this->_verifyMember( FALSE );
    }

    public function commentlist( $goodsid, $item, $nPage = 1 )
    {
        ...
        $this->pagedata['pager'] = array(
            "current" => $nPage,
            "total" => $aComment['page'],
            "link" => $this->system->mkUrl( "comment", "commentlist", array(
                $goodsid,
                $item,
                $tmp = time( )
            ) ),
    ...
    public function toComment( $goodsid, $item )
    {
        if ( $this->system->getConf( "comment.verifyCode.".$item ) == "on" && md5( $_POST[$item."verifyCode"] ) != $_COOKIE[strtoupper( $item )."_RANDOM_CODE"] )
        {
            if ( $item == "ask" )
            {
                $stp = __( "咨询" );
        ...
        else
        {
            ...
            $this->splash( "success", $this->system->mkUrl( "product", "index", array(
                $goodsid
            ) ), $message );
        }
    }

    public function csplash( $goodsid )
    {
        $this->splash( "success", $this->system->mkUrl( "product", "index", array(
            $goodsid
        ) ), "提交成功！" );
    ...
    public function reply( $comment_id, $item )
    {
        $objComment =& $this->system->loadModel( "comment/comment" );
        $aComment = $objComment->getFieldById( $comment_id, array( "*" ) );
        $aComment['reply'] = $objComment->getCommentsReply( array(
            $comment_id
        ), TRUE );
    ...
    public function toReply( $comment_id, $item )
    {
        if ( $this->system->getConf( "comment.verifyCode.".$item ) == "on" && md5( $_POST[$item."verifyCode"] ) != $_COOKIE[strtoupper( $item )."_RANDOM_CODE"] )
        {
            if ( $item == "ask" )
