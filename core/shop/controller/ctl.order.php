...
class ctl_order extends shopPage
{
    ...
    public function create( )
    {
        ...
        foreach ( $_POST['minfo'] as $k => $v )
        {
            foreach ( $v as $a => $b )
            {
                $GLOBALS['_POST']['minfo'][$k][$a]['value'] = strip_tags( $b['value'] );
            }
        }
        foreach ( $_POST['delivery'] as $kec => $kev )
        {
            $GLOBALS['_POST']['delivery'][$kec] = strip_tags( $kev );
        }
        ...
        {
            $GLOBALS['_POST']['delivery']['ship_addr'] = str_replace( "^\\s+|\\s+\$", "", $_POST['delivery']['ship_addr_area'].$_POST['delivery']['ship_addr'] );
        }
        ...
        $this->end_only( TRUE, __( "订单建立成功" ), $this->system->mkUrl( "order", "index", array(
            $orderid
        ) ) );
        $GLOBALS['GLOBALS']['pageinfo']['order_id'] = $orderid;
        $this->redirect( "order", "index", array(
            $orderid
        ) );
    }
