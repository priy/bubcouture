...
class ctl_brand extends shopPage
{
    ...
    public function showList( $page = 1 )
    {
        ...
        {
            $result[$k]['link'] = $this->system->mkUrl( "gallery", "index", array(
                "",
                $oSearch->encode( array(
                    "brand_id" => array(
                        $v['brand_id']
                    )
                ) )
        ...
        $this->pagedata['pager'] = array(
            ...
            "link" => $this->system->mkUrl( "brand", "showList", array(
                $tmp = time( )
            ) ),
            "token" => $tmp
        );
    ...
    public function index( $brand_id, $page = 1 )
    {
        ...
        $this->path[] = array(
            "title" => __( "品牌专区" ),
            "link" => $this->system->mkUrl( "brand", "showList" )
        );
        $this->path[] = array(
        ...
        $this->pagedata['pager'] = array(
            ...
            "link" => $this->system->mkUrl( "brand", "index", array(
                $brand_id,
                $tmp = time( )
            ) ),
            "token" => $tmp
        ...
        $oSearch =& $this->system->loadModel( "goods/search" );
        $this->pagedata['link'] = $this->system->mkUrl( "gallery", $this->system->getConf( "gallery.default_view" ), array(
            "",
            $oSearch->encode( array(
                "brand_id" => array(
                    $brand_id
    ...
    public function get_brand( &$result, $list = 0 )
    {
        if ( $list )
        {
    ...
    public function get_goods_amount( &$result, $list = 0 )
    {
        return $result['count'];
    }

    public function get_brand_intro( &$result, $list = 0 )
    {
        $brand_desc = preg_split( "/(<[^<>]+>)/", $result['data']['brand_desc'], -1 );
        if ( 50 < strlen( $brand_desc ) )
        {
    ...
    public function get_brand_kw( &$result, $list = 0 )
    {
