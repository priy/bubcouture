...
class entity_member extends entity
{
            ...
            case "contact" :
                foreach ( $v as $contact_v )
                {
                    foreach ( $contact_v as $ck => $cv )
                    {
                        ...
                        else if ( $ck == "firstname" || $ck == "lastname" )
                        {
    ...
    public function _schema_map( )
    {
        $member_schema = array(
            ...
            "meta" => array( "area", "mobile", "tel", "email", "zip", "addr", "province", "city", "order_num", "refer_id", "refer_url", "addon", "education", "vocation", "interest", "point_history", "reg_ip", "regtime", "state", "pay_time", "biz_money", "custom", "unreadmsg", "login_count", "experience", "foreign_id" ),
            "memo" => array( "remark" )
        );
        return $member_schema;
    }

    public function _getMember( $arr, $map_arr )
    {
        $array['member'] = array( );
    ...
    public function _getMemberId( $arr, $map_arr )
    {
        foreach ( $arr as $k => $v )
        {
    ...
    public function _getMemberGroupId( $arr, $map_arr )
    {
        foreach ( $arr as $k => $v )
        {
    ...
    public function _getMemberMeta( $arr, $map_arr )
    {
        $meta_arr['meta'] = array( );
        if ( is_array( $arr ) )
        {
    ...
    public function _getLang( $arr, $map_arr )
    {
        foreach ( $arr as $k => $v )
        {
    ...
    public function _getCurrency( $arr, $map_arr )
    {
        foreach ( $arr as $k => $v )
        {
            if ( in_array( $k, $map_arr['lang'] ) )
    ...
    public function _getAccount( $arr, $map_arr )
    {
        $account = array( );
        foreach ( $arr as $k => $v )
        {
    ...
    public function _getContact( $arr, $map_arr )
    {
        $tmp['phone'] = array( );
    ...
    public function _getProfile( $arr, $map_arr )
    {
    ...
    public function _getMemberStatus( $arr, $map_arr )
    {
        foreach ( $arr as $k => $v )
        {
    ...
    public function _getScore( $arr, $map_arr )
    {
        $score['score'] = array( );
        $tmp['event'] = array( );
        foreach ( $arr as $k => $v )
    ...
    public function _getAdvance( $arr, $map_arr )
    {
    ...
    public function _getMemo( $arr, $map_arr )
    {
        foreach ( $arr as $k => $v )
        {
