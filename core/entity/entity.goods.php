...
class entity_goods extends entity
{
    ...
    public function &export_sdf_array( $id )
    {
    ...
    public function import_sdf_array( &$sdf_array, $type )
    {
        $array = array( );
    default :
        switch ( $k )
        {
            ...
            case "adjunct" :
                ...
                foreach ( $v as $adjunct_k => $adjunct_v )
                {
                    ...
                    {
                        if ( 0 < count( $adjunct_v['adj_include'] ) )
                        {
                    ...
                    else if ( array_key_exists( "adj_filter", $adjunct_v ) && count( 0 < $adjunct_v['adj_filter'] ) )
                    {
                ...
                foreach ( $v as $product_v )
                {
                    ...
                    foreach ( $product_v as $pr_k => $pr_v )
                    {
                        ...
                        else if ( $pr_k == "spec_def" )
                        {
                            ...
                            $i = 0;
                            for ( ; $i < count( $tmppspecid['spec_value_id'] ); ++$i )
                            {
    ...
    public function _map_schema( )
    {
        $schema = array(
            ...
            "props" => array( "1" => "p_1", "2" => "p_2", "3" => "p_3", "4" => "p_4", "5" => "p_5", "6" => "p_6", "7" => "p_7", "8" => "p_8", "9" => "p_9", "10" => "p_10", "11" => "p_11", "12" => "p_12", "13" => "p_13", "14" => "p_14", "15" => "p_15", "16" => "p_16", "17" => "p_17", "18" => "p_18", "19" => "p_19", "20" => "p_20", "p_21", "p_22", "p_23", "p_24", "p_25", "p_26", "p_27", "p_28" ),
    ...
    public function _getGoodsInfo( $arr, $map_arr, $type_id )
    {
        $array['goods'] = array( );
    ...
    public function _getTitle( $arr, $map_arr )
    {
        foreach ( $arr as $k => $v )
        {
    ...
    public function _getOrderInfo( $arr )
    {
    ...
    public function _getProductNum( $arr )
    {
        $productsnum['products'] = 6;
        return $productsnum;
    }

    public function _getStatus( $arr )
    {
        foreach ( $arr as $k => $v )
        {
            if ( $k == "status" )
    ...
    public function _getGoodsAttributes( $arr, $map_arr )
    {
        foreach ( $arr as $k => $v )
        {
    ...
    public function _getGoodsId( $arr )
    {
        foreach ( $arr as $k => $v )
        {
            if ( $k == "goods_id" )
    ...
    public function _getBrand( $arr, $map_arr )
    {
    ...
    public function _getCategory( $arr, $map_arr )
    {
        foreach ( $arr as $k => $v )
        {
    ...
    public function _getCreatetime( $arr, $map_arr )
    {
        foreach ( $arr as $k => $v )
        {
    ...
    public function _getAdjunct( $arr, $map_arr )
    {
    ...
    public function _getMedia( $arr, $map_arr )
    {
            ...
            if ( in_array( $k, $map_arr['media'] ) )
            {
                if ( $k == "small_pic" )
                {
                    ...
                    $tmp = array(
                        "type" => $k,
                        "value" => $v,
                        "height" => $height_row['src_size_height'],
                        "width" => $width_row['src_size_width'],
                        "default" => 1,
                        "storage" => $smallatt[2],
                        "option" => "id=".$smallatt[1]
                    );
    ...
    public function _getThumbnail( $arr, $map_arr )
    {
        $th['thumbnail'] = array( );
        foreach ( $arr as $k => $v )
        {
    ...
    public function _getSpec( $arr, $map_arr )
    {
        $spec_arr['spec'] = array( );
        $option['option'] = array( );
        foreach ( $arr as $k => $v )
    ...
    public function _getLastmodified( $arr, $map_arr )
    {
        foreach ( $arr as $k => $v )
        {
    ...
    public function _getDescription( $arr, $map_arr )
    {
        foreach ( $arr as $k => $v )
        {
    ...
    public function _getMeta( $arr, $map_arr )
    {
        $meta_arr['meta'] = array( );
        if ( is_array( $arr ) )
        {
    ...
    public function _getProps( $arr, $map_arr, $type_id )
    {
        $array['props'] = array( );
        ...
        foreach ( $arr as $k => $v )
        {
            if ( !in_array( $k, $map_arr['props'] ) && is_null( $v ) )
            {
                $tmp[$k] = array(
                    "id" => $v,
                    "value" => $p[substr( $k, 2 )]['options'][$v]
                );
            }
        }
    ...
    public function _getBrief( $arr, $map_arr )
    {
        foreach ( $arr as $k => $v )
        {
    ...
    public function _getProduct( $arr, $map_arr )
    {
        $product['meta'] = array( );
        $product['price'] = array( );
        $product['store'] = array( );
                ...
                foreach ( $value as $k => $v )
                {
                    foreach ( $v as $kk => $vv )
                    {
                        ...
                        if ( in_array( $kk, $map_arr['product']['spec_value'] ) )
                        {
                            ...
                            $i = 0;
                            for ( ; $i < count( $vv['spec_value_id'] ); ++$i )
                            {
